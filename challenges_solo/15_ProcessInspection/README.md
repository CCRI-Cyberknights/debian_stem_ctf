...
**Mission Briefing:**
CryptKeepers operatives have deployed a covert process on a compromised system to quietly exfiltrate sensitive information. We were unable to capture the malware itself, but we managed to grab a **Process Snapshot** (`ps_dump.txt`) just before the connection closed. Somewhere in that list of running programs is a rogue agent executing a command with the flag passed as an argument.
...
* **The Strategy:** **Log Analysis**. We need to scan the "COMMAND" column of the snapshot for suspicious activity or sensitive strings.
...
| **less -S** | View the file manually. The `-S` flag prevents lines from wrapping. | `less -S ps_dump.txt` |
